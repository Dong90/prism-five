...
paradigm: Navigator
role: orchestrator
description: Prism-Five unified entry. Reads pipeline.json → preflight → gate check → dispatch to Agent.
---

<constraints>
SYSTEM OF RECORD: `.prism-five/pipeline.json` currentRole is the only state source.
...
4. Dispatch: load matching Agent SKILL.md from `.prism-five/agents/`

...
- **feat**: .opencode-plugin/: 新增独立目录与 manifest skeleton，注册 9 个 prism-* tool + 5 个 pentad-* skill
...
- **test**: 新增 4 个测试文件 / 44 个测试用例：status-json (3), agent (4), skill-frontmatter (35), opencode-plugin (5) — 总测试 45 → 89
...
- **观察期**: 1 week
- **观察指标**: OpenCode launch + slash command invocation; agent list render perf; status --json latency
- **退出标准**: no critical bugs; status --json latency under 50ms (NFR-PERF-01); 0 user-reported integration issues
...
**触发**: OpenCode plugin crash on launch OR critical command regression (status / continue / approve broken)
**操作**: 1. git revert HEAD; rm -rf .opencode-plugin/ 2. npm test; verify 89 tests pass after rollback
**时间**: ≤less than 10min

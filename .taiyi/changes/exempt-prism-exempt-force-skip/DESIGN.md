...
**触碰模块**:
- `packages/orchestrator/src/schema.ts`（既有 · 本次修改）
- `packages/orchestrator/src/upstream.ts`（新增）
- `packages/cli/src/commands/exempt.ts`（新增）
- `packages/orchestrator/src/audit.ts`（新增）
**禁动清单**:
- `packages/orchestrator/src/pipeline.ts`（AI 不许碰）
...
- **Chosen:** A
- **Reason:** 1. exempt 显式可审计
2. 过期时间是安全阀
3. --force-skip 与 exempt 互补
- **取舍:** 接受多一个 CLI 命令

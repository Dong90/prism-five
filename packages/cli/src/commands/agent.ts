...
const PENTAD_AGENT_PREFIX = 'pentad-';
...
export const agentCommand = new Command('agent')
  ...
  .argument('[name]', 'Agent name (e.g., pentad-rapid-prototyper)')
  ...
  .action((name, task: string[] | undefined) => {
    ...
    if (name.includes('/') || name.includes('..') || name.startsWith('-')) {
      console.error(`✗ Invalid agent name: "${name}". Must match pattern: ${PENTAD_AGENT_PREFIX}<role>`);
      process.exit(1);
    }
